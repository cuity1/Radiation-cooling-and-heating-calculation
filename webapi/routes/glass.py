...
router = APIRouter(tags=["glass"], dependencies=[Depends(require_user)])
...
@router.post("/glass/compare", response_model=CreateJobResponse)
def create_glass_comparison_job(req: CreateJobRequest, current_user: User = Depends(require_user)) -> CreateJobResponse:
    """创建玻璃对比分析任务（使用EnergyPlus）

    参数：
    - type: 必须是 "compare_glass"
    - params: 包含以下字段：
        - weather_group: "china"、"world" 或 "world_weather2025"
        - scenarios: 场景列表（至少3个），每个场景包含 name, desc, glass
    ...
    """
    print(f"\n[GLASS API] create_glass_comparison_job called")
    print(f"[GLASS API] Request type: {req.type}")
    print(f"[GLASS API] Request params keys: {list(req.params.keys()) if req.params else 'None'}")
    print(f"[GLASS API] Request params: {req.params}")

    if req.type != "compare_glass":
        print(f"[GLASS API] ERROR: Invalid job type: {req.type}")
        raise HTTPException(status_code=400, detail="Job type must be 'compare_glass'")
    if current_user.tier == "normal":
        # Map visualization / glass comparison is Pro-only
    ...
    # 打印收到的参数
    print(f"\n[API DEBUG] 创建玻璃对比任务: {job_id}")
    print(f"[API DEBUG] 原始参数: {req.params}")
    print(f"[API DEBUG] 任务备注: {req.remark}")
...
@router.get("/glass/{job_id}/files/{file_path:path}")
def get_glass_comparison_file(job_id: str, file_path: str, current_user: User = Depends(require_user)):
    """获取玻璃对比分析结果文件"""
    # 权限检查：仅管理员或任务所有者可访问
    with SessionLocal() as db:
        job = db.get(Job, job_id)
...
@router.get("/glass/climate-params")
def get_glass_climate_params(current_user: User = Depends(require_user)):
    """下载气候参数XLSX文件"""
    # 获取项目根目录
    base_dir = Path(__file__).resolve().parents[2]
    xlsx_path = base_dir / "glass_comparison_tool" / "气候参数.xlsx"
...
@router.get("/glass/model-preview/{model_key}/figure")
def get_glass_model_preview_figure(model_key: str, current_user: User = Depends(require_user)):
    """获取指定模型目录下的预览图片 figure.png。

    约定目录结构：
    - glass_comparison_tool/model/model1/figure.png
    - glass_comparison_tool/model/model2/figure.png
    - glass_comparison_tool/model/model3/figure.png
    """
    # 为安全起见，仅允许有限的模型键
    ...
    base_dir = Path(__file__).resolve().parents[2]
    fig_path = base_dir / "glass_comparison_tool" / "model" / model_key / "figure.png"
...
@router.get("/glass/model-preview/{model_key}/inf")
def get_glass_model_preview_inf(model_key: str, current_user: User = Depends(require_user)):
    ...
    base_dir = Path(__file__).resolve().parents[2]
    csv_path = base_dir / "glass_comparison_tool" / "model" / model_key / "inf.csv"
