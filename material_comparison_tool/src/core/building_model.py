...
class Surface:
    ...
    # 热容相关属性（用于动态热容模型）
    heat_capacity_per_area: float = 0.0  # J/K·m² - 单位面积热容
    thermal_resistance: float = 0.0  # m²·K/W - 总热阻
    thermal_mass_center_temp: float = 293.15  # K - 墙体核心温度（用于2R1C模型）
    temperature_prev: float = 293.15  # K - 上一时间步的表面温度
    ...
    def _calculate_thermal_properties(self):
        """根据构造层计算热容和热阻"""
        if not self.construction or not self.construction.layers:
            # 默认值（轻质结构）
            self.heat_capacity_per_area = 50000.0  # J/K·m²
            self.thermal_resistance = 2.0  # m²·K/W
            return
        ...
        for layer in self.construction.layers:
            ...
            if layer.conductivity > 0:
                r_layer = layer.thickness / layer.conductivity
                total_r += r_layer
            
            # 累加热容：C = ρ * cp * d（单位面积）
            # 对于2R1C模型，使用各层热容的加权平均（更偏向重质层）
            c_layer = layer.density * layer.specific_heat * layer.thickness
            total_c += c_layer
        
        self.thermal_resistance = max(0.1, total_r)  # 最小热阻 0.1 m²·K/W
        self.heat_capacity_per_area = max(10000.0, total_c)  # 最小热容 10000 J/K·m²
        # 初始化核心温度为当前表面温度
        self.thermal_mass_center_temp = self.temperature
