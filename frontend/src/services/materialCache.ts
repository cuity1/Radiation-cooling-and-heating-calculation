...
import type { MaterialComparisonParams, MaterialScenario, GlobalParams } from './materials'

/** 缓存版本号，用于数据迁移 */
const CACHE_VERSION = 1

/** localStorage 存储键 */
const STORAGE_KEY = 'material-comparison-params'
...
/** 缓存数据结构 */
interface MaterialParamsCache {
  version: number
  timestamp: number
  params: {
    weather_group: 'china' | 'world' | 'world_weather2025'
    idf_template_model: 'model1' | 'model2' | 'model3' | 'custom'
    custom_idf_template_dir: string
    enable_latent_heat: boolean
    wet_fraction: number
    scenarios: MaterialScenario[]
...
/** 默认场景配置 */
export function getDefaultScenarios(t?: (key: string) => string): MaterialScenario[] {
  const scenarioDesc = {
    baseline: t ? t('mapVisualization.scenarioDesc.baseline') : 'Baseline reference scenario',
    cooling: t ? t('mapVisualization.scenarioDesc.cooling') : 'Cooling-focused material',
    heating: t ? t('mapVisualization.scenarioDesc.heating') : 'Heating-focused material',
  }

  return [
    {
      name: 'duibi',
      desc: scenarioDesc.baseline,
      material: {
    ...
    {
      name: 'shiyan1',
      desc: scenarioDesc.cooling,
      material: {
        ...DEFAULT_MATERIAL_VALUES,
        Name: 'shiyan1',
        SolarAbsorptance: 0.1,
        ThermalAbsorptance: 0.95,
        VisibleAbsorptance: 0.1,
      },
    },
    {
      name: 'shiyan2',
      desc: scenarioDesc.heating,
      material: {
        ...DEFAULT_MATERIAL_VALUES,
        Name: 'shiyan2',
...
export function getDefaultColormapParams(weatherGroup: 'china' | 'world' | 'world_weather2025'): Record<string, string> {
  ...
  for (const [k, v] of Object.entries(defaultParams)) {
    if (k.startsWith(prefix) || k.startsWith(`world_weather2025`) && prefix === 'world_weather2025') {
 ...
 * 创建空缓存数据结构
 */
function createEmptyCache(): MaterialParamsCache {
  return {
    version: CACHE_VERSION,
    timestamp: Date.now(),
    params: {
      weather_group: 'china',
      idf_template_model: 'model1',
      custom_idf_template_dir: '',
 ...
 * 保存参数到 localStorage
 */
export function saveParamsToCache(params: MaterialParamsCache['params']): boolean {
  ...
  try {
    const cache: MaterialParamsCache = {
      version: CACHE_VERSION,
      timestamp: Date.now(),
      params,
    }
 ...
 * 从 localStorage 加载参数
 */
export function loadParamsFromCache(): MaterialParamsCache['params'] | null {
  if (!isLocalStorageAvailable()) {
    return null
  }
  ...
  try {
    ...
    const cache = JSON.parse(raw) as MaterialParamsCache
 ...
 * 防抖保存
 */
export function debouncedSave(params: MaterialParamsCache['params']): void {
  if (autoSaveTimer) {
 ...
 * 导出参数为 JSON 文件
 */
export function exportParams(params: MaterialParamsCache['params']): void {
  const exportData = {
    version: CACHE_VERSION,
  ...
  a.href = url
  a.download = `material-params-${new Date().toISOString().slice(0, 10)}.json`
 ...
 * 从文件导入参数
 */
export function importParamsFromFile(file: File): Promise<MaterialParamsCache['params'] | null> {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (data.version !== CACHE_VERSION) {
          console.warn('Import file has different version, may need migration')
        }

        if (data.params && data.params.scenarios) {
          resolve(data.params)
        } else {
...
export function getCacheStatus(locale: string = 'zh-CN'): CacheStatus {
  const timestamp = getCacheTimestamp()
  return {
    hasCache: hasCache(),
    timestamp,
    formattedTime: timestamp ? formatCacheTime(timestamp, locale) : null,
    locale,
  }
}
