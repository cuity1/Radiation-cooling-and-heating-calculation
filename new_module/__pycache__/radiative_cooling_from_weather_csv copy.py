...
def compute_cooling(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    ...
    # Calculate atmospheric downwelling radiation with fallback
    # 1. Use measured 'strd' (地表下行长波_W/m2) if available
    # 2. Fallback to physics model if 'strd' is missing
    ...
    # Net cooling power: emission - downwelling - absorbed solar
    out['净制冷功率_W/m2'] = out['材料热辐射_W/m2'] - out['大气向下辐射_W/m2'] - out['太阳吸收_W/m2']
...
def fig2_split(df, out_dir='figures/individual'):
    ...
    hourly2 = df.groupby('小时').agg({
        '材料热辐射_W/m2': 'mean',
        '大气向下辐射_W/m2': 'mean',
        '太阳吸收_W/m2': 'mean'
    })
    fig, ax = plt.subplots(figsize=BASE_FIGSIZE)
    ax.bar(hours, hourly2['材料热辐射_W/m2'], color=COLORS['red'], alpha=0.6, label='Thermal emission')
    ax.bar(hours, -hourly2['大气向下辐射_W/m2'], color=COLORS['blue'], alpha=0.6, label='Atmospheric absorption')
    ax.bar(hours, -hourly2['太阳吸收_W/m2'], bottom=-hourly2['大气向下辐射_W/m2'], color=COLORS['orange'], alpha=0.6, label='Solar absorption')
